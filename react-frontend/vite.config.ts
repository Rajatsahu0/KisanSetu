...
export default defineConfig({
  ...
  define: {
    // Define process.env for compatibility with libraries that use it
    'process.env': {}
  },
  plugins: [
    react(),
    VitePWA({
      ...
      workbox: {
        ...
        // Clean old caches on new SW activation — prevents stale asset blank screens
        ...
        runtimeCaching: [
          {
            urlPattern: /^https:\/\/api\.kisanmitra\.com\/.*/i,
    ...
    rollupOptions: {
      output: {
        manualChunks: {
          ...
          // Utilities
          'utils': ['clsx', 'tailwind-merge', 'papaparse', 'jspdf'],
        },
