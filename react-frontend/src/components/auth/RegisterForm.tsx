...
export const RegisterForm: React.FC<RegisterFormProps> = ({ onSuccess, onError }) => {
  ...
  const handleSendOtp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await authService.register(formattedPhone, '', name.trim());
      setShowOtp(true);
    } catch (err: any) {
      ...
      <form onSubmit={handleVerifyOtp} className="space-y-6">
        <div>
          <h3 className="text-lg font-medium text-gray-900 dark:text-gray-100 mb-2">
            {t('auth.confirmRegistration', 'Verify Phone Number')}
          </h3>
        ...
        >
          {isLoading ? <LoadingSpinner size="sm" /> : t('auth.verifyOtp', 'Verify & Register')}
        </button>
      </form>
    );
  }
